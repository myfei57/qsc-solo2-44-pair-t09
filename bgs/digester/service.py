...
class DigesterService(LineService):
    ...
    def observe_pressure(self, kpa: float) -> dict[str, Any]:
        ...
        if decision.should_latch and not self._latch.latched:
            ...
            self.publish(LATCH_KIND, {"active": True, "reason": event.reason})
            ...
            self.publish(CLEAR_KIND, {"active": False, "reason": event.reason})
    ...
    def _sync_latch(self) -> None:
        ...
        state = self.state()
        digester = state.get("digester", {})
        self._latch.sync(latched=latch_state(state, "digester"), step=0, reason="synced")
